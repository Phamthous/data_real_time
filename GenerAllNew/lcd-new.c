...
// перекодировочная таблица для русифицированных индикаторов
unsigned char __attribute__ ((progmem)) rus_table[64] = 
{
...
static void lcd_write(unsigned char data,unsigned char rs) 
{
	...
	else LCD_DATA2_PORT &= ~(1 <<LCD_DATA2_PIN);
	if(data & 0x20) LCD_DATA1_PORT |= (1 <<LCD_DATA1_PIN);
	else LCD_DATA1_PORT &= ~(1 <<LCD_DATA1_PIN);
	if(data & 0x10) LCD_DATA0_PORT |= (1 <<LCD_DATA0_PIN);   
	else LCD_DATA0_PORT &= ~(1 <<LCD_DATA0_PIN);
	lcd_e_toggle();
...
{
    register unsigned char addressCounter=0;
...
Returns:  none
*************************************************************************/
//void lcd_putc(char c) //sas
//int lcd_putc(char c, FILE *stream)
int lcd_putc(char c)	//sas to bee compatible with printf() ...
{
    unsigned char pos;
    ...
    else
    {
		...
		if(c < 0xc0)//sas	
			lcd_write(c, 1);//sas
		else//sas add russian symbols
			lcd_write(pgm_read_byte(&rus_table[c - 0xc0]),1);//sas
...
void lcd_init(unsigned char dispAttr)
{
    ...
    LCD_DATA1_PORT |= (1 <<LCD_DATA1_PIN);  // (1 <<LCD_FUNCTION)>>4;
    LCD_DATA0_PORT |= (1 <<LCD_DATA0_PIN);  // (1 <<LCD_FUNCTION_8BIT)>>4;
    lcd_e_toggle();
